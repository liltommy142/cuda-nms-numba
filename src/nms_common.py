"""Shared candidate contract and reference utilities for hard NMS variants.

Every hard-NMS implementation in this project consumes the same canonical
representation: ``boxes`` is float32 ``(N, 4)`` in ``xyxy`` format, ``scores``
is float32 ``(N,)``, and ``class_ids`` is int32 ``(N,)``.  Keeping the class
label explicit prevents accidental suppression across detector classes.
"""
...
def validate_candidates(boxes, scores, class_ids) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    Boxes must have positive area because all NMS variants assume valid ``xyxy``
    geometry.  Empty input is accepted when represented as an empty sequence.
    """
...
def stable_score_order(indices: np.ndarray, scores: np.ndarray) -> np.ndarray:
    """Order retained original indices by descending score, then input index."""
...
def stable_class_partitions(scores: np.ndarray, class_ids: np.ndarray) -> list[np.ndarray]:
    """Return per-class original indices in deterministic descending-score order."""
...
def load_synthetic_candidates(
    ...
    """Generate deterministic, multi-class ``xyxy`` candidates for NMS stress tests."""
    if n < 0:
...
def torchvision_class_aware_nms(
    ...
    """Apply torchvision greedy NMS independently to each class and re-sort."""
