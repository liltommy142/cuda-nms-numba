...
There is no public or repository-local Seminar 3 rubric. The user confirmed
that tonight's deliverable is a submission only, with no presentation. The
course material names three final products: a runnable report notebook, a team
plan, and GitHub commit history. The latest repository source is therefore the
authority for technical scope, while the course material defines the package.

Three options were considered:

1. **Reproducible submission bundle (selected):** ship a runnable notebook,
   source snapshot, team plan, current CUDA evidence, reproduction guide,
   checksums, and manifest in one ZIP.
2. **Code and raw logs only:** fastest, but it omits the course's runnable
   report requirement and leaves the grader to reconstruct the experiment.
3. **Long-form report and slide deck:** polished but unnecessary because the
   user explicitly said there is no presentation and the course requests an
   `.ipynb` report rather than slides.
