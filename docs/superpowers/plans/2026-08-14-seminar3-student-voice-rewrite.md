...
**Architecture:** Treat JSON/TXT evidence, commands, benchmark configuration, and machine-readable notebook strings as immutable interfaces. Add regression tests first, rewrite only human text, statically validate the three remote-cloning Colab notebooks, execute only the local final report, regenerate canonical Git-blob checksums, archive the committed allowlist, validate a fresh extraction, and safely sync the existing extracted copy.
...
- src/gpu_v1.ipynb; src/gpu_v2.ipynb; src/gpu_v3.ipynb — rewritten Markdown/labels only, statically validated as Colab workflows with unchanged code/output state.
- submission/seminar_3/FINAL_REPORT.ipynb — rewritten report Markdown/labels and the only notebook executed locally in the verified CUDA Python 3.11 environment.
...
COLAB_CODE_SOURCE_SHA256 = {
    ROOT / "src" / "gpu_v1.ipynb": "014e54b89b371a23efcacc0a119dca7fe9cf4c928787d561715af608ab47f893",
    ROOT / "src" / "gpu_v2.ipynb": "6074de9519497233cf510434b2a5105452cf29e9071ce34ea8cf658f3a9be361",
    ROOT / "src" / "gpu_v3.ipynb": "f50ea16bdbc5cedbbd68802d448ab91abc8179bb8ee97ee503ddbe1a66b4441a",
}
...
def test_colab_notebook_code_sources_match_pre_rewrite_fingerprints():
    import hashlib
    for path, expected in COLAB_CODE_SOURCE_SHA256.items():
        notebook = json.loads(path.read_text(encoding="utf-8"))
        normalized = "\n\n".join(
        ...
        assert hashlib.sha256(normalized.encode("utf-8")).hexdigest() == expected, path
...
### Task 4: Rewrite Colab presentation text statically; execute only the local final report

**Files:**
- Modify: src/gpu_v1.ipynb; src/gpu_v2.ipynb; src/gpu_v3.ipynb; submission/seminar_3/FINAL_REPORT.ipynb
- Modify: docs/superpowers/specs/2026-08-14-seminar3-submission-design.md
- Test: tests/test_submission_artifacts.py

**Interfaces:**
- Consumes: Task 1 test contract, original code cells, commands, and raw evidence.
- Produces: three statically validated Colab notebooks with Vietnamese human text and byte-semantically preserved code/output state, plus one locally executed final report.

- [ ] **Step 1: Snapshot the Colab immutable cells before editing**

Run the code-source regression test from Task 1 and capture the complete Colab code/output snapshot before editing:

~~~powershell
Copy-Item src\gpu_v1.ipynb "$env:TEMP\gpu_v1.before.ipynb" -Force
Copy-Item src\gpu_v2.ipynb "$env:TEMP\gpu_v2.before.ipynb" -Force
Copy-Item src\gpu_v3.ipynb "$env:TEMP\gpu_v3.before.ipynb" -Force
& .\.venv\Scripts\python.exe -m pytest tests\test_submission_artifacts.py -q -k "colab_notebook_code_sources_match_pre_rewrite_fingerprints"
~~~

Expected: PASS. The temporary snapshots are comparison inputs only and are not committed.

- [ ] **Step 2: Rewrite only titles, Markdown, comments, and human labels**

Use “nhóm em” in each. For the three src Colab notebooks, do not change imports, URLs, %cd, !git clone, !nvidia-smi, assertions, callable names, command arguments, JSON names, files.download, execution_count, or outputs. Their first cells clone remote code, change into that clone, require CUDA, and later import google.colab; never execute them locally. Preserve their stored code/output state byte-semantically, permitting only JSON formatting differences from the selected notebook editor. State: V1 checks IoU; V2 batch 32 still resolves greedily on CPU and cannot claim target success; V3 soft Matrix NMS uses matrix_nms_reference; FINAL_REPORT is CPU/V1/V2 hard-NMS evidence. Update the packaged voice design spec to state this safe distinction: validate the three Colab notebooks statically and run only FINAL_REPORT.ipynb locally in the verified CUDA Python 3.11 venv. Preserve exactly:
...
- [ ] **Step 3: Verify static Colab interfaces and preserved output state**

~~~powershell
& .\.venv\Scripts\python.exe -m pytest tests\test_submission_artifacts.py -q -k "each_notebook_uses_student_voice or colab_notebook_code_sources_match_pre_rewrite_fingerprints"
@'
import json
import os
from pathlib import Path
for name in ("gpu_v1", "gpu_v2", "gpu_v3"):
    before = json.loads((Path(os.environ["TEMP"]) / f"{name}.before.ipynb").read_text(encoding="utf-8"))
    after = json.loads((Path("src") / f"{name}.ipynb").read_text(encoding="utf-8"))
    assert [c.get("outputs") for c in before["cells"]] == [c.get("outputs") for c in after["cells"]], name
    assert [c.get("execution_count") for c in before["cells"]] == [c.get("execution_count") for c in after["cells"]], name
print("Colab output state: PASS")
'@ | & .\.venv\Scripts\python.exe
~~~

Expected: pytest PASS and `Colab output state: PASS`; no local Colab execution occurs.

- [ ] **Step 4: Execute only FINAL_REPORT.ipynb in place**
...
path = Path("submission/seminar_3/FINAL_REPORT.ipynb")
notebook = nbformat.read(path, as_version=4)
NotebookClient(notebook, timeout=600, kernel_name="python3", resources={"metadata": {"path": "."}}).execute()
nbformat.write(notebook, path)
print("executed: submission/seminar_3/FINAL_REPORT.ipynb")
'@ | & .\.venv\Scripts\python.exe
~~~

Expected: one executed line/no errors. CUDA absence is a real blocker; never fabricate successful output or substitute new evidence facts.
...
& .\.venv\Scripts\python.exe -m pytest tests\test_submission_artifacts.py -q
git add src\gpu_v1.ipynb src\gpu_v2.ipynb src\gpu_v3.ipynb submission\seminar_3\FINAL_REPORT.ipynb docs\superpowers\specs\2026-08-14-seminar3-submission-design.md
...
- [ ] Confirm the three Colab notebooks are statically validated with unchanged code-source fingerprints and output state; confirm only FINAL_REPORT.ipynb executes locally, remains valid JSON, retains PASS markers, and honestly shows MISSED.
