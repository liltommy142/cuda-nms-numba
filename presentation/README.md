...
**✅ Đã xác nhận nguồn gốc con số 0.2846s — pptx Slide 4 dùng đúng số trong proposal đã nộp, không phải số bịa/không rõ nguồn.** Đối chiếu trực tiếp với `CSC14116 - Proposal.docx` (mục "Measured CPU baseline timing" và "Profiling"): proposal ghi rõ N=100→0.0008s, N=1.000→0.0103s, N=10.000→**0.2846s**, và cProfile "total runtime 0.289s, ~65% (0.189s)... suppression loop, ~34% (0.098s)... iou_one_to_many". Đây **chính xác** là bộ số pptx Slide 4 dùng — không phải số dự kiến mơ hồ, mà trích thẳng từ proposal đã nộp và được giảng viên duyệt. Cách xử lý giữ nguyên như đã làm: `OUTLINE_AND_CONTENT.md` Slide 4 giữ bảng này làm số liệu "đo trong proposal" (motivation ban đầu), phần kết quả benchmark chính thức dùng số Colab verify sau này (2.4918s, 65% IoU/35% suppression) — 2 bộ số hợp lệ song song vì khác ngữ cảnh đo, không mâu thuẫn.

**⚠️ Gap thật phát sinh từ việc đối chiếu này**: proposal trích dẫn `profile_output/cprofile_N10000.txt` làm bằng chứng cho số liệu cProfile ở trên — **file này không tồn tại trong repo** (đã kiểm tra, không có thư mục `profile_output/` nào). Nếu giảng viên muốn tái kiểm chứng số cProfile gốc trong proposal, sẽ không tìm thấy file được trích dẫn. File thật gần nhất hiện có là [`cprofile_N10000_local.txt`](cprofile_N10000_local.txt) (chạy lại sau này, số khác — xem đoạn trên), không phải file gốc trong proposal.
